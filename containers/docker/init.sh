...
#!/bin/bash
/etc/init.d/$INITD_NAME restart
while [ `ps -C $WAIT_ON_PROCESS_NAME -o pid= | wc -l` -gt 0 ]; do
	sleep 30
done
EOF
...
for service in dovecot rsyslogd; do
...
# provided by the base image. We're running as process 1, and
# /sbin/my_init needs to run as process 1, so use 'exec' to replace
